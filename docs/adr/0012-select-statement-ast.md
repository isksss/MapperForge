...
- `joins`: top-level `JOIN` 句を kind、table、ON expression として保持する
- `windows`: top-level `WINDOW` 句を name と spec として保持する
- `fetch`: top-level `FETCH FIRST/NEXT` 句を raw と count expression として保持する

Parser は top-level の `FROM`、`JOIN`、`WHERE`、`GROUP BY`、`HAVING`、`ORDER BY`、`LIMIT`、`OFFSET`、`WINDOW`、`FETCH` を句境界として扱う。select list や comma 区切り句は括弧と配列リテラル内の comma を無視して分割し、各 item を `SqlExpressionParser` に渡す。

## Consequences

SQL formatter、validator、printer が SELECT の主要部分を Expression AST として参照できるようになる。方言固有句は今後 golden file test を追加しながら段階的に AST 化する。
