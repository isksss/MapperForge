...
Parser は top-level の `INTO`、`VALUES`、`SET`、`FROM`、`WHERE` を句境界として扱い、値や条件式は `SqlExpressionParser` に渡す。既存互換のため、raw のみを受け取る constructor は残す。

## Consequences

DML の placeholder、代入値、WHERE 条件を AST として検証・printer へ渡せるようになる。複数行 `VALUES`、`INSERT ... SELECT`、`RETURNING`、`DELETE ... USING`、方言固有句は golden file test を追加しながら段階的に拡張する。
